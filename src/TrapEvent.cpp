...
TrapEvent::TrapEvent(std::string desc)
    : description_(std::move(desc)) {}

void TrapEvent::execute(Player& player) {
    std::cout << "\n=== " << title << " ===\n";
    ...
    player.loseLives(1);
    std::cout << " You lose 1 life! (" << currentLives << " Health:️ -> " << player.getLives() << " ️)\n";
...
std::string TrapEvent::getDescription() const {
    return description_;
}